...
// --- Unit tests: probeCheck ---

func TestProbeCheck_healthy_when_marker_exists(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".healthy")
	...
	got := probeCheck(path)
	if got != 0 {
		t.Errorf("probeCheck(marker exists) = %d, want 0", got)
	}
}

func TestProbeCheck_unhealthy_when_marker_absent_and_dir_writable(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".healthy")
...
func TestProbeCheck_degraded_when_marker_absent_and_dir_not_writable(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "ro")
	if err := os.Mkdir(dir, 0o500); err != nil {
	...
	got := probeCheck(path)
	if got != 0 {
		t.Errorf("probeCheck(degraded) = %d, want 0 (degraded reports healthy)", got)
	}
}
